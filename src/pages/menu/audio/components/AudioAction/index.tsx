...
const AudioAction: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    if (props.audioList && props.audioList.length !== 0 && playIndex !== -1) {
      ...
      let cid = props.audioList[playIndex].cid;
      let newUrl = getCidUrl(cid, props.audioList[playIndex].file_name);
      if (newUrl === url) {
        resetAudio();
      } else {
        setUrl(newUrl);
      ...
      return (
        <>
          ...
          <div className="infoBox">
            ...
            <div>
              <div className="title">CID:&nbsp;</div>
              <div className="value pointer" onClick={goCheck}>
                <Popover content={info.cid}>{info.cid.substr(0, 25) + '...'}</Popover>
              </div>
            </div>
          </div>
        </>
