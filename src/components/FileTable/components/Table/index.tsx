...
import { useInfiniteScroll, useLatest, useSetState } from 'ahooks';
...
const Table: React.FC<Props> = (props) => {
      ...
      callback(index: any) {
        ...
        if (props.title == '最近使用') {
          if (typeof index === 'number') {
            deleteRecentlyList([data?.list[index].cid]);
            props.reloadTable();
          ...
          let folderName = data?.list[0].file_folder[data?.list[0].file_folder.length - 1];
          if (typeof index === 'number') {
            props.setClickItem({
            ...
            props.setClickItem({ listData: index, folderName: folderName, isOnly: false });
            props.setDeleteModal(true);
          }
        }
      },
  ...
  let toggleChecked = (index: number) => {
    ...
    for (const key in listCopy) {
      checkList.push(data?.list[key].cid);
    }
  ...
  let widthProcess = (res: number | undefined) => {
    if (res) {
      let rate = Math.floor((res / (1130 - 56)) * 100);
      return {
        minWidth: rate + '%',
      };
    } else {
  ...
  const nameEditIndexRef = useLatest(nameEditIndex);
      ...
      let trRender = (item: any) => {
        let absoluteSetRender = () => {
          ...
          let lines = setList.map((item, setListIndex: number) => {
            return (
              <div className={styles.line} key={setListIndex} onClick={() => setListClick(item)}>
                <Iconfont type={item.icon} size={16}></Iconfont>
              </div>
            );
          });
          return <div className={styles.setAbsoluteBox}>{lines}</div>;
        };
        let nameRender = () => {
          let r = null;
          if (nameEditIndex.index === index) {
            r = (
              <div
                className={`${styles.nameBox} ${styles.editInputNameBox}`}
                onClick={(e) => {
                  e.stopPropagation();
                }}
              >
                <Input
                  placeholder=""
                  onPressEnter={(e) => changeName(e)}
                  bordered={false}
                  autoFocus
                  defaultValue={item.file_name}
                  onBlur={() => {
                    setTimeout(() => {
                      if (nameEditIndexRef.current.index === index) {
                        setNameEditIndex({ ...nameEditIndex, index: -1 });
                      }
                    }, 300);
                  }}
                />
        ...
        >
          <div
            className={`${styles.tableList} ${
              tableCheckedList[index] ? styles.tableListActive : ''
            }`}
          >
